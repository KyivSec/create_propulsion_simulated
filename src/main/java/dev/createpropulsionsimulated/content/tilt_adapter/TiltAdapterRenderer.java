...
public class TiltAdapterRenderer extends KineticBlockEntityRenderer<TiltAdapterBlockEntity> {
	...
	protected void renderSafe(TiltAdapterBlockEntity blockEntity, float partialTicks, PoseStack ms, MultiBufferSource buffer, int light, int overlay) {
        ...
        SuperByteBuffer inputShaft = CachedBuffers.partialFacing(CPSPartialModels.TILT_ADAPTER_INPUT_SHAFT, blockState, invDirection);
        SuperByteBuffer outputShaft = CachedBuffers.partialFacing(CPSPartialModels.TILT_ADAPTER_OUTPUT_SHAFT, blockState, invDirection);
    ...
    private void renderOverlays(TiltAdapterBlockEntity blockEntity, BlockState blockState, Direction invDirection, Vector3f aom, int light, PoseStack ms, MultiBufferSource buffer, Direction direction, boolean alignedX, boolean positive) {
        ...
        SuperByteBuffer gantry = CachedBuffers.partialFacing(CPSPartialModels.TILT_ADAPTER_GANTRY, blockState, invDirection);
        SuperByteBuffer sideOverlay = CachedBuffers.partialFacing(CPSPartialModels.TILT_ADAPTER_SIDE_INDICATOR, blockState, invDirection);
    ...
    private void renderOverlaySide(float redstoneSignal, float angle, SuperByteBuffer gantry, SuperByteBuffer sideOverlay, int light, PoseStack ms, MultiBufferSource buffer, Direction direction, boolean isRight, boolean alignedX, boolean positive) {
        ...
        net.minecraft.core.Direction.Axis rotationAxis = isHorizontal ? net.minecraft.core.Direction.Axis.Y : net.minecraft.core.Direction.Axis.Z;
    ...
    private void applyLocalTransforms(PoseStack ms, boolean alignedX, boolean shouldFlip, net.minecraft.core.Direction.Axis rotationAxis) {
        ...
        if (shouldFlip) {
            ms.mulPose(rotationAxis == net.minecraft.core.Direction.Axis.Y ? ROT_Y_180 : ROT_Z_180);
        }
        ms.translate(-0.5, -0.5, -0.5);
    }
